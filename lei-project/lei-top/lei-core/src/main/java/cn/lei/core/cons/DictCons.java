...
public class DictCons {
    ...
    /**
     * 保险保单状态
     * 10已出单（保单已生成，还未进入保障中）、 20保障中（进入了起保时间，未到终保时间）、 30、判陪中（保障期结束进入判赔中，时间不确定，在安信未推送理赔结果之前一直是判赔中） 40、未发生理赔（保险保障期，发起理赔，保险公司判处结果，最终无需赔付） 41、判陪成功
     * 50、理赔成功（保险保障期，发起理赔，并理赔成功）
     */
