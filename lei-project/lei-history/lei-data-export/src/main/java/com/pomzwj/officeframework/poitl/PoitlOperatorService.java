...
public class PoitlOperatorService {
    ...
    public XWPFTemplate makeDoc(String dbKind, List<DbTable> tableList) throws Exception {
        ...
        RowRenderData headerRow = Rows.of(headerList.toArray(new String[headerList.size()])).textColor("FFFFFF")
                .bgColor("4472C4").create();
        ...
        tempMap.put("seg", Includes.ofLocal(templateCopyPath + "/" + TemplateFileConstants.SUB_MODEL_TEMPLATE)
                .setRenderModel(segmentDataList).create());
    ...
    public List<RowRenderData> getRow(DbTable dbTable, List<String> columnNames, RowRenderData headerRow)
            throws Exception {
