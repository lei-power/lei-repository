...
FROM (SELECT table_name 表名, column_name 字段名, column_comment 字段注释 FROM information_schema.`COLUMNS` WHERE table_schema = 'gmall_user')
         AS a
         LEFT JOIN
...
FROM (SELECT table_name 表名, column_name 字段名, column_comment 字段注释 FROM information_schema.`COLUMNS` WHERE table_schema = 'test')
         AS a
         LEFT JOIN
